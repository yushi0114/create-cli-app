 ...
 */
export async function selectFeature(projectName: string): Promise<Questions> {
  // 清空命令行
  clearConsole();
  // 输出信息
  ...
  const questions: Questions = await prompt([
    {
      type: 'input',
 ...
 */
export function changePackageInfo(
  projectName: string,
  description: string,
): void {
...
export function installFeature(feature: Array<string>): void {
  feature.forEach((item) => {
    const func = installFeatureMethod[`install${item}`] as unknown as (
